...
// read as the other.
var abandonTag = []byte("dcr4inarow/movelog/abandon/v1")
...
type Abandon struct {
	...
	Head [32]byte
	Seq  uint64
	// Board is which board was in play.
	Board uint8
...
func (a *Abandon) SigningBytes() ([]byte, error) {
	...
	_ = binary.Write(&b, binary.BigEndian, a.Board)
...
func (c *Chain) Abandon(key *forfeit.LogKey, seat, stalled uint32, reason Reason, boardIndex uint8, deadline uint32) (*Abandon, error) {
	...
	head, seq := c.Head()
	a := &Abandon{
		Version: Version, Head: head, Seq: seq, Board: boardIndex,
...
func (a *Abandon) MarshalJSON() ([]byte, error) {
	...
	return json.Marshal(abandonJSON{
		...
		Board: a.Board, Seat: a.Seat, Stalled: a.Stalled, Reason: uint8(a.Reason),
		Deadline: a.Deadline,
...
func DecodeAbandon(blob []byte) (*Abandon, error) {
	...
	a := &Abandon{
		Version: in.Version, Seq: in.Seq, Board: in.Board, Seat: in.Seat,
