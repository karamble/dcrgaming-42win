...
func TestATamperedColumnBreaksTheSignature(t *testing.T) {
	c, keys := table(t)
	e, err := c.Move(keys[0], 0, 0, 3, 100)
	if err != nil {
		t.Fatalf("move: %v", err)
	...
	e.Column = 4
	if err := e.Verify(); err == nil {
		t.Fatal("a move played into another column still verified")
	}
}
...
func TestATranscriptRebuildsFromTheRosterAlone(t *testing.T) {
	...
	for i, seat := range []uint32{0, 1, 0, 1} {
		if _, err := c.Move(keys[seat], seat, 0, uint8(i), 100); err != nil {
...
func TestATranscriptWithATamperedMoveIsRefused(t *testing.T) {
	...
	tampered := bytes.Replace(blob, []byte(`"column":3`), []byte(`"column":4`), 1)
	if bytes.Equal(tampered, blob) {
		t.Fatal("the test did not change the column it meant to")
	}
...
func TestEquivocationAtOneMoveNumberPublishesTheKey(t *testing.T) {
	...
	entry := func(column uint8) *movelog.Entry {
	...
	if hashA == hashB {
		t.Fatal("two different columns hashed the same")
	}
...
func TestTwoDifferentAbandonmentsAtOnePositionPublishTheKey(t *testing.T) {
	...
	build := func(deadline uint32) *movelog.Abandon {
		return &movelog.Abandon{
			...
			Board: 0, Seat: 0, Stalled: 1, Reason: movelog.ReasonStall,
...
func TestAJournalReplaysAMatch(t *testing.T) {
	...
	for i, seat := range []uint32{0, 1, 0, 1} {
		e, err := c.Move(keys[seat], seat, 0, uint8(i), 100)
		if err != nil {
			t.Fatalf("move %d: %v", i, err)
		}
...
func TestAReservedMoveSurvivesToBeSignedAgain(t *testing.T) {
	...
	intended := &movelog.Entry{
		Version: movelog.Version, PrevHash: head, Seq: seq, Board: 0, Seat: 0,
		Column: 3, Height: 100, Signer: keys[0].Public().SerializeCompressed(),
	}
	...
	if rep.Pending.Column != 3 || rep.Pending.Seq != seq {
		t.Fatalf("the reserved move came back as column %d at %d", rep.Pending.Column, rep.Pending.Seq)
	}
	if rep.Chain.Len() != 0 {
