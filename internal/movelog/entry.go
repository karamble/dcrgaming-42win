...
// position and nothing else - so telling one peer that move seven was column
// three and another that it was column four produces two signatures over one
...
// inside a match: boards are a field in an entry, never a reset of the counter.
//
...
// a question about four-in-a-row, and answering it here would drag the rules
...
var (
	entryTag = []byte("dcr4inarow/movelog/entry/v1")
	matchTag = []byte("dcr4inarow/movelog/match/v1")
)
...
type Entry struct {
	Version  uint16
	PrevHash [32]byte
	// Seq is the move number within the whole match, across all its boards.
...
func (e *Entry) SigningBytes() ([]byte, error) {
	...
	_ = binary.Write(&b, binary.BigEndian, e.Board)
	_ = binary.Write(&b, binary.BigEndian, e.Seat)
	_ = binary.Write(&b, binary.BigEndian, e.Column)
