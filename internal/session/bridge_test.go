...
func standTwo(t *testing.T) (*bridgetest.Bridge, [2]*peer) {
	...
	fake := bridgetest.New(bridgetest.Options{
		Game: session.GameID, Network: "mainnet", Params: params, Height: 800,
	})
...
func TestAWholeMatchIsPlayedAndSettledOverABridge(t *testing.T) {
	...
	for i, p := range peers {
		if err := p.rt.Fund(ctx, tableSID); err != nil {
			t.Fatalf("peer %d funding its stake: %v", i, err)
		}
	}
	...
	for board := 0; board < 3; board++ {
		for _, col := range openerWinsIn7 {
			view, _ := bySeat[0].game.View(tableSID)
			if view.Done {
				t.Fatal("the match ended before the third board")
			}
			if err := bySeat[view.Turn].game.Play(ctx, tableSID, col); err != nil {
				t.Fatalf("board %d, column %d: %v", board, col, err)
			}
	...
	if !first.Won || first.Winner != session.OpenerSeat {
		t.Fatalf("seat %d was paid; seat %d opened boards one and three",
			first.Winner, session.OpenerSeat)
	}
