// Package session is dcr4inarow's side of the SDK runtime.
//
...
//
// A move deadline alone does not bound a match. A best-of-three is up to 126
// moves, so even at an hour each two seats could keep a table open for days
...
var SeatTags = identity.SeatTags{
	Session: "4inarow/session/v1",
	Log:     "4inarow/log/v1",
	Bond:    "4inarow/bond/v1",
}

// OpenerSeat is the seat that opens the first board.
//
...
// Game is dcr4inarow's rules, and the state of every table it has open.
type Game struct {
	dir string
...
func (g *Game) open(sid string) error {
	...
	g.tables[sid] = t
	if chain.Len() > 0 {
		log.Infof("table %s: resumed at move %d, board %d, seat %d to play",
...
func (g *Game) handleMove(t *table, body []byte) error {
	...
	mv := match.Move{Board: e.Board, Seat: e.Seat, Column: e.Column}
...
// money: it turns a board somebody is losing into a void that returns their
...
// wrong board or a deadline this log does not imply is rejected outright: it
...
func (g *Game) handleAbandon(t *table, body []byte) error {
	...
	if int(a.Board) != t.play.Index() {
		return fmt.Errorf("it names board %d; this match is on board %d", a.Board, t.play.Index())
	}
...
// which is a way to abandon a losing board immediately.
...
// to leave a losing board without playing it. That it can still leave a losing
// board by waiting out the deadline is an accepted limitation of this release,
...
func (g *Game) Play(ctx context.Context, sid string, column uint8) error {
	rt, err := g.runtime()
	if err != nil {
		return err
	}
	...
	if err := t.play.CanPlay(match.Move{Board: boardIndex, Seat: t.seat, Column: column}); err != nil {
		g.mu.Unlock()
		return err
	}
	...
	intended := &movelog.Entry{
		...
		Board: boardIndex, Seat: t.seat, Column: column, Height: uint32(tip.Height),
	...
	e, err := t.chain.Move(key, t.seat, boardIndex, column, uint32(tip.Height))
	if err != nil {
		g.mu.Unlock()
		return err
	}
	...
	if err := t.play.Play(match.Move{Board: boardIndex, Seat: t.seat, Column: column}); err != nil {
		g.mu.Unlock()
		return err
	}
	g.mu.Unlock()
...
func (g *Game) State(_ context.Context) sdk.State {
	...
	for sid, t := range g.tables {
		status := fmt.Sprintf("board %d, seat %d to move", t.play.Index()+1, t.play.Turn())
		if t.play.Done() {
